...
*Covering SRIT v1.7 · DAI v1.6 · DDA v1.4*
...
**Operational documents — use these to run the tools:**

| Module | Document 1 | Document 2 | Document 3 |
|--------|-----------|-----------|-----------|
| SRIT | `SRIT_Claude_SysDesc_v1_0` | `SRIT_ChatGPT_RunSetup_v1_0` | `SRIT_Claude_Review_v1_0` |
| DAI | `DAI_Claude_InputPrep_v1_1` | `DAI_ChatGPT_RunSetup_v1_1` | `DAI_Claude_Review_v1_1` |
| DDA | `DDA_Claude_InputPrep_v1_1` | `DDA_ChatGPT_RunSetup_v1_1` | `DDA_Claude_Review_v1_1` |
...
Used for three functions across the pipeline: system description preparation before SRIT, input record validation before DAI and DDA, and declaration/judgement review between Stage 1 and Stage 2 of the final prompt in each module. Not used for execution runs.
...
Use `SRIT_Claude_SysDesc` to develop these four components before beginning the ChatGPT run. The five minutes spent on boundary precision prevents hours of analytical drift.
...
Use `DDA_Claude_InputPrep` to confirm the DAI Run Record is complete and the Readiness gate is satisfied before opening ChatGPT for the DDA run.
...
Used for preparation and review steps only, not execution. Sonnet 4.6 is the correct model for these steps — they are conversational and interrogative, not analytical. No scarcity, denial, or drift analysis occurs in Claude sessions.

Session discipline: use a new Claude session or continue the preparation session for the review step. There is no context dependency between the preparation session and the review — the Stage 1 output is self-contained.
