...
SRIT is designed to execute as a single uninterrupted run after user input is collected at Prompt 1.

The execution sequence is:

1. **Prompt 1** — Ask the four structured questions and collect user responses. Produce the System Definition Statement. This is the only point at which user input is required.
2. **Prompts 2 through 6** — Execute sequentially without pausing for further user input. Each prompt operates strictly on the outputs of the previous prompt.
3. **Run Record** — Produce the SRIT Run Record at the conclusion of Prompt 6.

Do not pause between prompts. Do not request confirmation between stages. Do not ask the user whether to proceed. Execute the full sequence from Prompt 2 to the Run Record as a single continuous output once Prompt 1 is complete.

#### Mode Selection

Before beginning, select the execution mode appropriate to your purpose:

**Single-pass mode** — Default for experienced users on familiar system types. The AI executes all prompts as a single continuous run after Prompt 1 user input. No stopping between prompts. Use this mode for routine analytical runs.

**Sequential review mode** — For protocol development, validation work, or high-stakes first runs on unfamiliar system types. The AI stops after each prompt and waits for explicit instruction before proceeding. Use this mode when you want to review each output before the next prompt fires — particularly at Prompt 4, where an incorrect system boundary or unresolved mechanism/resource distinction should be caught before the analysis proceeds to evidence verification and declaration.

State your chosen mode to the AI before beginning Prompt 1. If no mode is stated, the AI defaults to single-pass mode.
...
**User input required:** Briefly describe the system you want to analyse. A few words or a sentence is sufficient — the AI will interrogate the description to establish the correct boundaries.

> *Example: "Global container shipping network" or "A regional hospital system" or "National electricity grid"*
...
Your role is not to identify the scarce resource immediately.

Your role is to interrogate the system description so that the correct analytical boundaries are established.

Ask structured questions to clarify the following:

1. The system being analysed
2. The boundary of the system
3. The time horizon of stability being considered
4. The condition that constitutes system failure

Ask questions sequentially and do not attempt to identify any scarcity yet.

To assist boundary selection, the AI may offer **bounded illustrative options** within these four variables only.

If insufficient information is provided, ask follow-up questions before producing the System Definition Statement.

Return a concise **System Definition Statement**.

When this Prompt 1 has successfully completed, request Prompt 2.

### Expected AI Behaviour

The AI should ask clarification questions only to establish:

- the system under analysis
- the system boundary (what is included and excluded)
- the time horizon of stability
- the condition that represents system failure

The AI must not:

- propose candidate scarcities
- begin analysis of scarcity
...
When this Prompt 2 has successfully completed, request Prompt 3.
...
When this Prompt 3 has successfully completed, request Prompt 4.
...
When this Prompt 4 has successfully completed, request Prompt 5.
...
For **every surviving candidate**, apply the following check to the weakening evidence:

> Does the weakening evidence for this candidate describe its failure pathway as operating mainly by causing another resource to be leaked, delayed, withheld, misallocated, left unmobilised, or left unprotected?

Apply the following rule:

- **If no:** continue with evidence review and ranking.
- **If yes:** pause ranking for that candidate and return it to **Prompt 4 Mechanism Gate Step 2**, using the full Prompt 5 evidence record as additional input.

When returning a candidate to Mechanism Gate Step 2 from Prompt 5:

- do not rerun the whole of Prompt 4
- apply only the Standalone Failure Test using the fuller evidence record
- if direct standalone resource character is demonstrated, the candidate remains on the ranking
- if direct standalone resource character is not demonstrated, classify the candidate as **Mechanism — eliminate** under **R6**, remove it from the Prompt 5 ranking, and log it as a **candidate mechanism-layer item for DAI**
...
> **Candidate: Skilled Labour**
>
> Behavioural Evidence
> - Expected signal: training pipeline expansion
> - Observed signal: labour shortages during peak demand
>
> Structural Evidence
> - Institutional structures: certification system, training capacity limits
>
> Stress Evidence
> - System behaviour: throughput collapse if labour unavailable

When this Prompt 5 has successfully completed, request Prompt 6.
...
When this Prompt 6 has successfully completed, inform the user that this SRIT run is now complete.
...
[For each candidate subjected to the mandatory late-catch retest in Prompt 5, record:
- **Candidate:** [name]
- **Late-catch trigger:** [one line description of the weakening evidence that triggered the retest]
...
If no candidate triggered the late-catch retest, state: Late-catch retest applied to all candidates — no mechanism character identified.]
